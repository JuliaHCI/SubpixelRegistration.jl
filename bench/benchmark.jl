...
@progress "sizes" for N in SIZES
    ...
    @progress "upsample factors" for ups in UPSAMPLE_FACTORS
        ...
        bench_py = @benchmark skr.phase_cross_correlation(
            $img,
            $img_shifted,
            upsample_factor = $ups,
            return_error = true,
        ...
        bundle = (;
            size = N,
            upsample_factor = ups,
            time_jl = median(bench_jl).time / 1e9,
            time_py = median(bench_py).time / 1e9,
        )
        @info "" bundle...
        push!(rows, bundle)
    end
...
outpath =
    joinpath(dirname(pathof(SubpixelRegistration)), "..", "bench", "benchmark_results.csv")
